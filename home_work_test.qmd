...
summary_table_1 <- data.frame(
  Category = c("Smoking", "Alcohol", "Physical Activity"),  # Categories to summarize
  Yes = c(sum(df$smoke == "yes"), sum(df$alco == "yes"), sum(df$active == "yes")),  # Count of "yes" responses
  No = c(sum(df$smoke == "no"), sum(df$alco == "no"), sum(df$active == "no"))  # Count of "no" responses
)

# Add a total column for each category
summary_table_1$Total <- summary_table_1$Yes + summary_table_1$No

####################
# Create a detailed summary table considering the 'cardio' factor
summary_table_2 <- df %>%
  group_by(cardio) %>%
  summarize(
  ...
  mutate(
    Total_Smoking = Smoking_Yes + Smoking_No,
    Total_Alcohol = Alcohol_Yes + Alcohol_No,
    Total_Active = Active_Yes + Active_No
  )
...
kable_table_func(overview_table, "propotion for each cardio group")
```
