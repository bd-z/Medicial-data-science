...
# # Create a styled table with centered column names and content
summary_data %>%
  kable("html", caption = "<b>Summary of Data</b>", row.names = FALSE) %>% # Bold caption
  kable_styling(
    "striped",                             # Add striped rows
    full_width = F,                        # Table width fits content
    position = "center",                   # Center-align the table
    fixed_thead = F                        # Keep header fixed when scrolling
  ) %>%
 # add_header_above(c(" " = 1, "Summary Statistics" = 3)) %>%
  column_spec(1:ncol(summary_data),  extra_css = "text-align: center;") %>%  # Center-align all columns
  row_spec(0, align = "center", bold = TRUE, background = "#D3D3D3")                      # Center-align column headers
  ...
  for (var in continuous_variables) {
    ...
    boxplot <- ggplot(data, aes(y = .data[[var]])) +
      ...
      theme(
        ...
        axis.line = element_line(linewidth = 0.5),  # 替代 size 参数
...
knitr::kable(summary(data_clean[continuous_variables]), format = "latex", booktabs = TRUE, longtable = FALSE)
